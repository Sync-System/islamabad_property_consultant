      ...
      <Container>
        <div className="grid gap-14 lg:grid-cols-12 lg:gap-16">
          ...
          <div className="lg:col-span-6">
            <Reveal>
              <p className="flex items-center gap-3.5">
                ...
                <span className="eyebrow text-brass-700">The consultancy</span>
              </p>
              <h2 id="agency-title" className="optical-left mt-5 text-h2 text-ink-900">
                {agencyConfig.name}
              </h2>
              <p className="mt-6 max-w-[46ch] text-lead text-ink-700">
                {agencyConfig.positioning}
              </p>
            </Reveal>

            <Reveal delay={100}>
              <p className="mt-6 max-w-[58ch] text-body text-ink-600">
            ...
            {/* Consultant card */}
            <Reveal delay={160}>
              <div className="mt-10 flex items-center gap-5 border border-ink-900/12 bg-paper-50 p-5">
                <div className="relative grid size-20 shrink-0 place-items-center overflow-hidden border border-ink-900/12 bg-paper-100 sm:size-24">
                  {resolved(agencyConfig.consultantPhoto) ? (
                    ...
                    <span
                      aria-hidden="true"
                      className="font-display text-h3 leading-none text-ink-500"
                    >
                      IPC
                    </span>
                  )}
                ...
                <div>
                  <p className="eyebrow text-ink-500">{agencyConfig.consultantRole}</p>
                  <p className="mt-2 font-display text-h4 text-ink-900">
            ...
            <ul className="mt-10 grid gap-x-8 gap-y-7 sm:grid-cols-2">
              {COMMITMENTS.map((item, index) => (
                <Reveal as="li" key={item.title} delay={index * 80}>
                  <h3 className="border-t border-ink-900/15 pt-4 font-sans text-body-sm font-semibold text-ink-900">
                    {item.title}
                  </h3>
                  <p className="mt-2 text-body-sm text-ink-600">{item.body}</p>
                </Reveal>
              ))}
            </ul>
            ...
            {/* Contact */}
            <Reveal delay={200}>
              <dl className="mt-10 grid gap-5 border-t border-ink-900/12 pt-8 sm:grid-cols-2">
                <div>
                  ...
                  <dd className="mt-2">
                    <WhatsAppLink
                      ...
                      bare
                      hideIcon
                      className="text-body font-medium text-ink-900 underline decoration-brass-500/60 underline-offset-4 hover:decoration-brass-600"
                    >
                ...
                <div>
                  ...
                  <dd className="mt-2 text-body text-ink-700">
                    {email ? (
                      <a
                        href={`mailto:${email}`}
                        className="font-medium text-ink-900 underline decoration-brass-500/60 underline-offset-4"
                      >
                        {email}
                      </a>
                    ) : (
                      <span className="text-ink-500">To be confirmed</span>
                    )}
                  </dd>
                </div>
                <div>
                  <dt className="eyebrow text-ink-500">Office</dt>
                  <dd className="mt-2 flex items-start gap-2 text-body-sm text-ink-700">
                    <PinIcon size={17} className="mt-0.5 shrink-0 text-ink-500" />
                    {office ? (
                      mapUrl ? (
                        <a
                          href={mapUrl}
          ...
          <Reveal delay={140} className="lg:col-span-6" id="contact">
            <div className="border border-ink-900/12 bg-paper-50 p-6 sm:p-9 lg:sticky lg:top-[calc(var(--header-h)+2rem)]">
              <h3 className="font-display text-h3 text-ink-900">
                Ask a consultant
              </h3>
              <p className="mt-3 max-w-[42ch] text-body-sm text-ink-600">
